...
def terabox(url) -> str:
    try:
        ...
        res = client.get(
            f"https://www.terabox.com/share/list?jsToken={jsToken}&shorturl={key}&root=1",
            cookies=cookies,
        )
        result = res.json()
    except Exception as e:
    ...
    if result["errno"] != 0:
        raise DirectDownloadLinkException(f"ERROR: '{result['errmsg']}' Check cookies")
    result = result["list"]
    if len(result) > 1:
    ...
    try:
        ...
        raise DirectDownloadLinkException(f"ERROR: {e}, Check cookies")
    return dlink
